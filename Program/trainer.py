...
class HMIProgressCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        progress = int(((epoch + 1) / 5) * 100); self.state['training_progress'] = progress

def train_local_model(model_name, state, num_classes=5):
    base_dir = "dataset"
    img_size = (224, 224); batch_size = 32; state['training_progress'] = 0
    ...
    try:
        ...
        print(f"Trainer Error: {e}"); return False